...
DEFINE_LUA_FUNCTION(memory_isvalid, "address")
{
	unsigned long address = luaL_checkinteger(L,1);
	lua_settop(L,0);
	lua_pushboolean(L, PowerPC::HostIsRAMAddress(address));
	return 1;
}
...
DEFINE_LUA_FUNCTION(state_save, "location[,option]")
{
	...
	switch(type)
	{
		...
		{
			unsigned char* stateBufferPtr = (unsigned char*)lua_touserdata(L,1);
			if(stateBufferPtr)
			{
				stateBufferPtr += ((16 - (u64)stateBufferPtr) & 15); // for performance alignment reasons
				std::vector<u8> stateBuffer;
				State::Flush();
				State::SaveToBuffer(stateBuffer);
				memcpy(stateBufferPtr, &stateBuffer[0], stateBuffer.size());
			}
			return 0;
		}	
	}
	...
	switch(type)
	{
		...
		case LUA_TUSERDATA: // in-memory save slot
		{
			...
			if(stateBufferPtr)
			{
				...
				if(stateBufferPtr[0])
				{
					std::vector<u8> stateBuffer(stateBufferPtr, stateBufferPtr + State::GetSize());
					State::LoadFromBuffer(stateBuffer);
				}
...
DEFINE_LUA_FUNCTION(state_verify, "location")
{
	...
	switch(type)
	{
		...
		{
			State::Flush();
			State::Verify((int)luaL_checkinteger(L,1));
			return 0;
		}
		...
		case LUA_TUSERDATA: // in-memory save slot
		{
			...
			if(stateBufferPtr)
			{
				...
				{
					std::vector<u8> stateBuffer(stateBufferPtr, stateBufferPtr + State::GetSize());
					State::VerifyBuffer(stateBuffer);
				}
...
DEFINE_LUA_FUNCTION(gui_getpixel, "x,y")
{
	...
	x = std::max(0,min(xres,x));
	y = std::max(0,min(yres,y));
...
DEFINE_LUA_FUNCTION(emulua_loadrom, "filename")
{
	...
	// It might be possible to replace most of this function with BootManager::BootCore(filename)

	SConfig& StartUp = SConfig::GetInstance();

	StartUp.m_BootType = SConfig::EBootType::BOOT_ISO;
	...
	if (!StartUp.AutoSetup(SConfig::EBootBS2::BOOT_DEFAULT)) return 1;
	...
	if (StartUp.GetUniqueID().size() == 6)
	{
		...
		// General settings
		core_section->Get("CPUOnThread",	  &StartUp.bCPUThread, StartUp.bCPUThread);
...
{
	lua_pushinteger(L, Movie::IsPlayingInput() || Movie::IsRecordingInput() ? (lua_Integer)Movie::g_currentFrame : 0);
	return 1;
}
...
// Since the name of a movie is no longer stored while playing,
// this function cannot exist as it originally functioned
...
// TODO: define a function in Movie.cpp to accomplish this
